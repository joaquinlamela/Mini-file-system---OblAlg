...
ListaPos<Cadena>* Cadena::split(const char * token)
{
	ListaPos<Cadena>* cad = new ListaPosImp<Cadena>;
	char* aux = strtok(this->GetNewCharPtr(), token);
	while (aux != NULL)
	{
		cad->AgregarFin(aux);
		aux = strtok(NULL, "/");
...
Cadena Cadena::cadenaDesdePos(unsigned int posLinea) {
	if (posLinea > this->Length())
	{
		Cadena* c = new Cadena();
		return *c;
	}
	else {
		int largo = this->Length() - posLinea + 2;
		char* shift = new char[largo];
		for (int i = 0; i < largo - 1; i++)
		{
			shift[i] = (*this)[i + posLinea - 1];
		}
		shift[largo - 1] = '\0';
		const char* aRetornar = shift;
		Cadena * c = new Cadena(aRetornar);
		delete[] shift;
		return *c;
	}
...
Cadena Cadena::cadenaHastaPos(unsigned int posLinea) {
	char * auxHastaPos = new char(posLinea);
	for (int i = 0; i < posLinea - 1; i++)
	{
		auxHastaPos[i] = (*this)[i];
	}
	auxHastaPos[posLinea - 1] = '\0';
	Cadena * c = new Cadena(auxHastaPos);
	delete[] auxHastaPos;
	return *c;
}
...
void Cadena::EliminarEnCadena(unsigned int posLinea, unsigned int k) {
	*this = this->cadenaHastaPos(posLinea) + this->cadenaHastaPos(posLinea + k); 
}
